...
import 'package:viraeshop_admin/reusable_widgets/loading_widget.dart';
import 'package:viraeshop_admin/screens/customers/tabWidgets.dart';

class SupplierTaskScreen extends StatefulWidget {
  final String? orderId;
  const SupplierTaskScreen({Key? key, this.orderId}) : super(key: key);

  @override
  _SupplierTaskScreenState createState() => _SupplierTaskScreenState();
}

class _SupplierTaskScreenState extends State<SupplierTaskScreen> {
  ...
  void _onSearchChanged() {
    if (widget.orderId == null) return; // Search logic for list is handled by the Bloc if needed, but here we just filter details
    ...
    setState(() {
      if (query.isEmpty) {
        ...
        _filteredGroupedItems = _groupedItems.map((group) {
          final items = (group['items'] as List).where((item) {
            final productName = (item is Items ? item.productName : item['productName']).toString().toLowerCase();
            return productName.contains(query);
          }).toList();
          
          if (items.isNotEmpty || group['supplierName'].toString().toLowerCase().contains(query)) {
            return {
              ...group,
  ...
  double _calculateTotal() {
    ...
    for (var group in _groupedItems) {
      for (var itemData in group['items']) {
        ...
        if (item.processingStatus == 'confirmed' && (item.availability ?? true)) {
  ...
  Widget build(BuildContext context) {
    return widget.orderId == null ? _buildOrderList(context) : _buildPreparationDetail(context);
  }
            ...
            child: Container(
              ...
              child: TextField(
                decoration: InputDecoration(
                  hintText: "Search preparation orders...",
  ...
  // --- Detail View ( Grouping Logic ) ---
  Widget _buildPreparationDetail(BuildContext context) {
    return BlocListener<OrderItemsBloc, OrderItemState>(
      ...
      child: Scaffold(
                    ...
                    child: Container(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          prefixIcon: const Icon(Icons.search, color: Color(0xFF8B5E3C), size: 20),
                          border: InputBorder.none,
        ...
        body: _isLoading
            ...
            : _errorMessage != null
                ? Center(child: Text(_errorMessage!, style: GoogleFonts.inter()))
                : ListView.builder(
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: const Icon(Icons.storefront_rounded, size: 20, color: Color(0xFF8B5E3C)),
                ),
                ...
                Container(
                  ...
                  child: Text(
                    "${items.length} Items",
                    style: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.bold, color: const Color(0xFF8B5E3C)),
                  ),
                ),
              ],
            ),
                ...
                Row(
                  children: [
                    Text(
                      ...
                      style: GoogleFonts.outfit(color: const Color(0xFF8B5E3C), fontWeight: FontWeight.bold, fontSize: 13),
                    ),
  ...
  Widget _buildStatusDropdown(Items item) {
    final currentStatus = item.processingStatus == 'confirmed' ? 'Confirm' : (item.availability == false ? 'Fail' : 'Select');
    final statusColor = currentStatus == 'Confirm' ? const Color(0xFF10B981) : (currentStatus == 'Fail' ? const Color(0xFFEF4444) : Colors.grey[600]);
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          value: currentStatus,
          icon: Icon(Icons.keyboard_arrow_down_rounded, size: 18, color: statusColor),
          style: GoogleFonts.inter(fontSize: 12, color: statusColor, fontWeight: FontWeight.bold),
          ...
          children: [
            Row(
              ...
              children: [
                ...
                Column(
                  children: [
                    _buildLogisticsIcon(FontAwesomeIcons.truck, "Vira Van", active: true),
                    const SizedBox(height: 6),
                    Text(
                ...
                GestureDetector(
                  onTap: () {
                    final allIds = _groupedItems.expand((g) => g['items'] as List).map((i) => (i is Items ? i.id : i['id'])).toList();
                  ...
                  child: Container(
                    ...
                    child: const Icon(FontAwesomeIcons.paperPlane, color: Colors.white, size: 22),
                  ),
                ),
              ],
            ),
            ...
            Row(
              ...
              children: [
                ...
                Text(
                  ...
                  style: GoogleFonts.outfit(fontSize: 28, fontWeight: FontWeight.bold, color: const Color(0xFF8B5E3C)),
                ),
              ],
            ),
          ],
  ...
  Widget _buildLogisticsIcon(IconData icon, String label, {bool active = false}) {
    return Column(
      children: [
        Container(
          padding: const EdgeInsets.all(12),
          decoration: BoxDecoration(
            color: active ? const Color(0xFF8B5E3C).withOpacity(0.1) : Colors.grey[50]?.withOpacity(0.5),
            borderRadius: BorderRadius.circular(15),
            border: active ? Border.all(color: const Color(0xFF8B5E3C).withOpacity(0.3), width: 1.5) : Border.all(color: Colors.grey[200]!),
          ),
          child: Icon(icon, color: active ? const Color(0xFF8B5E3C) : Colors.grey[400], size: 24),
        ),
        const SizedBox(height: 8),
        Text(
          label,
          textAlign: TextAlign.center,
          style: GoogleFonts.inter(fontSize: 11, color: active ? const Color(0xFF1E293B) : Colors.grey[400], fontWeight: active ? FontWeight.w600 : FontWeight.w500),
        ),
      ],
    );
  }
